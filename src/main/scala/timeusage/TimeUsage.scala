...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def classifyColumn(groups: (List[Column], List[Column], List[Column]), column: String): (List[Column], List[Column], List[Column]) = {
      val prefix = column.take(3)
      if(primaryActivities.exists(column.startsWith))
        (col(column)::groups._1, groups._2, groups._3)
      else if(workingActivities.exists(column.startsWith))
        (groups._1, col(column)::groups._2, groups._3)
      else if(otherActivities.exists(column.startsWith))
        (groups._1, groups._2, col(column)::groups._3)
      else groups
    }
    columnNames.foldLeft((List[Column](), List[Column](), List[Column]()))(classifyColumn)
  }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when($"teage".between(14.0, 22.0), "young").when($"teage".between(23.0, 55.0), "active").otherwise("elder").as("age")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(row => (row.working, row.sex, row.age))
      .agg(
        ...
        typed.avg(_.work),
        typed.avg(_.other))
      .map({case ((working, age, sex), primaryNeeds, work, other) => TimeUsageRow(working, age, sex, math.round(primaryNeeds), math.round(work), math.round(other))})
        .orderBy("working", "sex", "age")
  }
}
